...
wiod_sea = pd.read_excel(
    "input/WIOD_SEA_Nov16.xlsx", sheet_name="DATA", engine="openpyxl"
)
employed = (
    wiod_sea.query("variable == 'EMP'")
    .groupby("country", as_index=False)[2014]
    .sum()
    .rename(columns={"country": "out_country", 2014: "employed_i"})
)
row = pd.DataFrame(
    {"out_country": ["ROW"], "employed_i": [employed["employed_i"].sum()]}
)
...
trade_flows = trade_flows.groupby(ijk_cols, as_index=False)[["value"]].sum()
...
z_ik["out_ind"] = (
    z_ik["delta_ik"].str.split("_").apply(lambda x: x[1]).astype("int64")
)
# Merge back into trade_flows as this is our main dataset. Both z_ik and z_jk
trade_flows = trade_flows.merge(
    z_ik[["out_country", "out_ind", "z_ik"]],
    how="left",
    left_on=jk_cols,
    right_on=ik_cols,
)
trade_flows.rename(
    columns={"z_ik": "z_jk", "out_country_x": "out_country"}, inplace=True
)
trade_flows.drop("out_country_y", axis=1, inplace=True)
trade_flows = trade_flows.merge(
    z_ik[["out_country", "out_ind", "z_ik"]], how="left", on=ik_cols
)
...
alpha_jk = wiot.groupby(["in_country", "in_ind"], as_index=False)["value"].sum()
alpha_jk["alpha_jk"] = alpha_jk.groupby("in_country", as_index=False)[
    "value"
...
pi_ijk["pi_ijk"] = trade_flows.groupby(
    ["in_country", "out_ind"], as_index=False
)["value"].transform(lambda x: x / sum(x))
# Compute pi_jjk since its useful to estimate trade costs
pi_jjk = pi_ijk[pi_ijk["out_country"] == pi_ijk["in_country"]]
pi_ijk = pi_ijk.merge(
    pi_jjk.drop("out_country", axis=1), on=["in_country", "out_ind"], how="left"
)
pi_ijk.rename(
    columns={"pi_ijk_x": "pi_ijk", "pi_ijk_y": "pi_jjk"}, inplace=True
)
...
lambda_ij = lambda_ij.groupby(ij_cols, as_index=False)["lambda_ij"].sum()
# Creates the Lambda matrix
Lambda_mat = lambda_ij.pivot(
    index="out_country", columns="in_country", values="lambda_ij"
)
# Vector of gamma guesses
n_countries = Lambda_mat.shape[0]
gamma_old = pd.Series(
    np.ones(n_countries) / n_countries, index=Lambda_mat.index
)
...
gamma_i["wage_i"] = gamma_i["gamma_i"] * wld_wage.values / gamma_i["employed_i"]
# Check gamma_i dataframe
gamma_i
# Merge everything back to trade_flows, the main database
trade_flows = (
    trade_flows.merge(alpha_jk, on=jk_cols, how="left")
    .merge(pi_ijk, on=ijk_cols, how="left")
    .merge(gamma_i, on=["out_country"], how="left")
    .merge(gamma_i, left_on="in_country", right_on="out_country", how="left")
)
trade_flows.drop("out_country_y", axis=1, inplace=True)
trade_flows.rename(
    columns={
        "out_country_x": "out_country",
        "gamma_i_x": "gamma_i",
...
# Clean environment
# del (alpha_jk, pi_ijk, gamma_i, gamma_old, gamma_new, Lambda_mat, lambda_ij,
# usa_idx, row, usa_idx, z_ik)

# Computing trading costs
trade_flows = trade_flows.assign(
    delta_ijk=lambda x: x["pi_ijk"] / x["pi_jjk"],
    z_ratio_ijk=lambda x: x["z_ik"] / x["z_jk"],
    w_ratio_ij=lambda x: x["wage_i"] / x["wage_j"],
)
trade_flows = trade_flows.assign(
    d_ijk=lambda x: x["delta_ijk"] ** (-1 / theta)
    * x["z_ratio_ijk"]
    / x["w_ratio_ij"]
)
...
def pred_value(df):
    return (
        (((df["wage_i"] * df["d_ijk"] / df["z_ik"]) ** (-theta)) / df["Phi_jk"])
        * df["alpha_jk"]
        * df["wage_j"]
        * df["employed_j"]
    )
...
def pred_pi(df):
    return ((df["wage_i"] * df["d_ijk"] / df["z_ik"]) ** (-theta)) / df[
        "Phi_jk"
    ]
